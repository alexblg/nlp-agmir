...
class ReportTransformer2(nn.Module):
...
class ReportTransformer(nn.Module):
    ...
    def forward(self, src, tgt, src_key_padding_mask, tgt_key_padding_mask, memory_key_padding_mask, tgt_mask):
        device = self.embed_tgt.weight.device
        src = rearrange(torch.cat((
            rearrange(src[0], 'n s -> s n')
            ,rearrange(torch.zeros([src[0].shape[0], 5]).to(device, dtype=src.dtype), 'n s -> s n')
        )), 's n -> n s').unsqueeze(0).to(device, dtype=torch.float32)
        tgt = rearrange(tgt, 'n t -> t n')
        tgt = self.pos_enc(self.embed_tgt(tgt) * math.sqrt(self.d_model))
